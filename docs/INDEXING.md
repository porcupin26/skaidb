...
| Global | `CREATE INDEX i ON t (a) WITH (global = true)` | (phase 1: write-path entry plumbing only — no reads yet) | entries value-sharded on the ring; see [GLOBAL_INDEXES.md](GLOBAL_INDEXES.md) |
